...
class signal_filtered:
	...
	def average(self, new_data):
		...
		signal = ( self.q6 + self.q5 +self.q4 + self.q3 + self.q2 + self.q1)/6
...
def torque_sinpercycle(freq, start_time):
	joint_torq_desired = 0.001*np.sin(freq*(time.time() - start_time))# + np.random.randn(1)*0.1
...
if __name__ == '__main__':
	...
	model = load_model('model_mixed_joint4_linear4.h5')
	# Variables
	NUM_DATA = 2000
	joint1_en = False
	joint2_en = False
	...
	joint7_en = False
	amplitude = 0.1
	DIRECTORY = '../data/sawyer/vel_acel_ns/test_joint4/'
	FILENAME = DIRECTORY + 'torque_test_mixed_joint4.csv'
	...
	for itera in range(NUM_DATA):
		...
		if joint4_en == True:
			...
			joint4_desired = signal.average(robot.joint_data[4] + model.predict(np.array([[robot.vel_data[4], 10.]]))) #signal.average(robot.joint_data[4] + model.predict(np.array([[robot.vel_data[4], 0]])))
		...
		df_tmp = pd.DataFrame([[joint1_pos, joint2_pos, joint3_pos, joint4_pos, joint5_pos, joint6_pos, joint7_pos, velocity1, velocity2, velocity3, velocity4, velocity5, velocity6, velocity7, joint1_desired, joint2_desired, joint3_desired, joint4_desired[0][0], joint5_desired, joint6_desired, joint7_desired, effort1, effort2, effort3, effort4, effort5, effort6, effort7]], columns=column_index)
