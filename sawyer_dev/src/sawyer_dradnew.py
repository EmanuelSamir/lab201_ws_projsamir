...
def motion_drad(fa, fb, t, Aa = 7., Ab=0.02):
...
if __name__ == '__main__':
	...
	FILENAME = DIRECTORY + 'quasi_static_02_500.csv'
	...
	for itera in range(NUM_DATA):
		if joint1_en == True:
			...
			"""	
			joint1_desired = joint_f1.send_motion(robot.joint_data[1], 1.6, 1., time.time()-start_time1)
			#en1a = enable2nextcycle(freq1a, time.time()-start_time1a)
			#en1b = enable2nextcycle(freq1b, time.time()-start_time1b)
		...
		if joint2_en == True:
			"""
			if en2a == False:
				start_time2a = time.time()
				freq2a = random_frequency_sat()
			if en2b == False:
				start_time2b = time.time()
				freq2b = random_frequency_sat()
			"""	
			joint2_desired = joint_f2.send_motion(robot.joint_data[2], 1.2, 1., time.time()-start_time2)
			#en2a = enable2nextcycle(freq2a, time.time()-start_time2a)
			#en2b = enable2nextcycle(freq2b, time.time()-start_time2b)
		...
		if joint3_en == True:
			...
			"""	
			joint3_desired = joint_f3.send_motion(robot.joint_data[3], 1.2, 1., time.time()-start_time3)
			#en3a = enable2nextcycle(freq3a, time.time()-start_time3a)
			#en3b = enable2nextcycle(freq3b, time.time()-start_time3b)
		...
		if joint4_en == True:
			"""
			if en4a == False:
				start_time4a = time.time()
				freq4a = random_frequency_sat()
			if en4b == False:
				start_time4b = time.time()
				freq4b = random_frequency_sat()
			"""	
			joint4_desired = robot.joint_data[4] + 0.02
			#en4a = enable2nextcycle(freq4a, time.time()-start_time4a)
			#en4b = enable2nextcycle(freq4b, time.time()-start_time4b)
		...
		if joint5_en == True:
			"""
			if en5a == False:
				start_time5a = time.time()
				freq5a = random_frequency_sat()
			if en5b == False:
				start_time5b = time.time()
				freq5b = random_frequency_sat()
			"""	
			joint5_desired = joint_f5.send_motion(robot.joint_data[5], 2.1, 1., time.time()-start_time5)
			#en5a = enable2nextcycle(freq5a, time.time()-start_time5a)
			#en5b = enable2nextcycle(freq5b, time.time()-start_time5b)
		...
		if joint6_en == True:
			"""
			if en6a == False:
				start_time6a = time.time()
				freq6a = random_frequency_sat()
			if en6b == False:
				start_time6b = time.time()
				freq6b = random_frequency_sat()
			"""	
			joint6_desired = joint_f6.send_motion(robot.joint_data[6], 2.1, 1., time.time()-start_time6)
			#en6a = enable2nextcycle(freq6a, time.time()-start_time6a)
			#en6b = enable2nextcycle(freq6b, time.time()-start_time6b)
		...
		if joint7_en == True:
			"""
			if en7a == False:
				start_time7a = time.time()
				freq7a = random_frequency_sat()
			if en7b == False:
				start_time7b = time.time()
				freq7b = random_frequency_sat()
			"""	
			joint7_desired = joint_f7.send_motion(robot.joint_data[7], 1.2, 1., time.time()-start_time7)
