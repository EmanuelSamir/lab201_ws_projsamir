...
def motion_sinpercycle(start_time):
	# Parameters
	A_0 = 0.01
	A_1 = 0.01
	f = 2.1

	joint_pos_desired = (A_0 + A_1*(time.time() - start_time))*np.sin(f*(time.time() - start_time))# + np.random.randn(1)*0.1
...
if __name__ == '__main__':
	...
	joint7_en = False
	amplitude = 0.1
	DIRECTORY = '../data/sawyer/temp/'#vel_sin_motion_incrf/test_joint4/'
	FILENAME = DIRECTORY + 'test_incrementalsin_joint4.csv'
	...
	for itera in range(NUM_DATA):
		if joint1_en == True:
			joint1_desired = motion_sinpercycle(start_time1)
		...
		if joint2_en == True:
			joint2_desired = motion_sinpercycle(start_time2)
		...
		if joint3_en == True:
			joint3_desired = motion_sinpercycle(start_time3)
		...
		if joint4_en == True:
			joint4_desired = motion_sinpercycle(start_time4)
		...
		if joint5_en == True:
			joint5_desired = motion_sinpercycle(start_time5)
		...
		if joint6_en == True:
			joint6_desired = motion_sinpercycle(start_time6)
		...
		if joint7_en == True:
			joint7_desired = motion_sinpercycle(start_time7)
