...
def motion_sinpercycle(freq1, freq2, start_time):
	_t0 = time.time() - start_time
	joint_pos_desired = 0.5*np.sin(freq1*(_t0 + 0.02*np.sin(freq2*_t0)))# + np.random.randn(1)*0.1
...
def random_frequency_sat(saturation_min = 0.05, saturation_max = 1.25):
	...
	#"""
	while True:
		random_freq = (saturation_max - saturation_min)*np.random.rand()#np.random.randn()
...
if __name__ == '__main__':
	...
	FILENAME = DIRECTORY + 'training_data_drad_new.csv'
	...
	for itera in range(NUM_DATA):
		if joint1_en == True:
			...
			joint1_desired = sign1*motion_sinpercycle(freq1,freq1b, start_time1)
		...
		if joint2_en == True:
			...
			joint2_desired = sign2*motion_sinpercycle(freq2,freq2b, start_time2)
		...
		if joint3_en == True:
			...
			joint3_desired = sign3*motion_sinpercycle(freq3,freq3b, start_time3)
		...
		if joint4_en == True:
			...
			joint4_desired = sign4*motion_sinpercycle(freq4,freq4b, start_time4)
		...
		if joint5_en == True:
			...
			joint5_desired = sign5*motion_sinpercycle(freq5,freq5b, start_time5)
		...
		if joint6_en == True:
			...
			joint6_desired = sign6*motion_sinpercycle(freq6,freq6b, start_time6)
		...
		if joint7_en == True:
			...
			joint7_desired = sign7*motion_sinpercycle(freq7,freq7b, start_time7)
